...
!
if (nr.eq.2) write(29,*) 'Particle Tracking'
    do while(dt_total.lt.dt_comp.and.nx_part.gt.0)
       nx_s=nx_s+1
       x_part(nx_s)=x_dist(nr,nx_part-1)
       ncell=segment_cell(nr,nx_part)
       if (nr .eq. 2) write(29,*) 'ncell ',ncell,nr,ns,nx_s,nx_part       
       dt_part(nx_s)=dx(ncell)/u(ncell) 
       dt_total=dt_total+dt_part(nx_s)
       if (dt_total .gt. dt_comp) then
         dt_xcess = dt_total - dt_comp
         dt_part(nx_s) = dt_part(nx_s)-dt_xcess
         x_part(nx_s) = x_dist(nr,nx_part) + u(ncell)*dt_part(nx_s)
       end if
!
!     Decrement the segment counter if the total time is less than the
!     computational interval
!
        nx_part=nx_part-1
    end do
